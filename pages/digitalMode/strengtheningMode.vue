<template>
	<view class="reading-training">
		<Navigation :title='"强化训练"'></Navigation>
		<view class="explain" v-if="showStatus===0">
			...
			<input type="text" v-model="myNumber" class="input" placeholder="请输入两位数字" />
			<view class="start-button" @click="changeShowStatus(1,0)">
				随机训练
			</view>
			<view class="start-button" @click="changeShowStatus(1,1)">
				顺序训练
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeShowStatus(e, n) {
				console.log(this.myNumber.length)
				if (isNaN(this.myNumber) || this.myNumber < 0 || this.myNumber > 100 || this.myNumber.length !== 2) {
					uni.showToast({
						icon: 'none',
						title: '必须是00-99两位数字'
					})
					...
					this.showStatus = e
					if (e === 1) {
						this.startCountownDNumber(n)
					}
				}
			},
			startCountownDNumber(n) {
